    ...
    client.connect(function(err) {
      ...
      try {
        ...
        client.query(existsQuery, (err, res) => {
          ...
          if (rowCount===0) {
            let createQuery = `CREATE TABLE todo(id SERIAL PRIMARY KEY, task VARCHAR(140) UNIQUE, done INT DEFAULT 0);`
...
app.post('/api/todo', async (req, res) => {
  ...
  try {
    ...
    connect({ servers: process.env.NATS_URL }).then(async (nats) => {
      const stringCodec = StringCodec();
      nats.publish("messages", stringCodec.encode("new todo was created"));
    })
    res.status(201).send()
  } catch (error) {
    console.log(error)
...
app.put('/api/todo', async (req, res) => {
  ...
  try {
    ...
    await client.query(`UPDATE todo SET done=1 WHERE task=${todo};`);
...
app.get('/healthz', async (req, res) => {
  let selectQuery = `SELECT * FROM todo;`;
  console.log('healthz request received')
  client.query(selectQuery, (err, result) => {
   if (err) {
      console.error(err);
      res.status(500).send(err)
      ...
      console.log('returning 200 for healthz')
      res.status(200).send()
    }
  })
})  
